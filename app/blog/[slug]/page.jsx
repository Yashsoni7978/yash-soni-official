...
export default async function BlogPost({ params }) {
  ...
  const articleSchema = {
    ...
    image: post.image ? `https://yashsoni.in${post.image}` : "https://yashsoni.in/og-image.webp",
  ...
  return (
    <>
      ...
      <div className="h-[60vh] md:h-[75vh] w-full relative overflow-hidden bg-[#0a0a0a]">
        {post.image && (
          <Image
            src={post.image}
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-[#050505] via-[#050505]/40 to-transparent flex items-end p-6 md:p-20 z-10">
          <div className="max-w-5xl">
            ...
            <h1 className="text-3xl md:text-5xl lg:text-6xl font-black text-white uppercase leading-[1.05] tracking-tight drop-shadow-2xl">
              {post.title}
            </h1>
          </div>
        </div>
      ...
      {/* Blog Content */}
      <article
        className="max-w-3xl mx-auto py-16 md:py-24 px-6 prose prose-lg md:prose-xl prose-invert prose-headings:font-black prose-headings:uppercase prose-p:font-light prose-a:text-[#B5952F] prose-strong:text-white"
      ...
      >
        ...
        <div className="flex flex-wrap gap-4 md:gap-6 mb-12 text-zinc-400 font-bold uppercase text-[10px] tracking-widest border-b border-white/10 pb-8 not-prose">
          ...
          <span className="flex items-center gap-2 mt-1">
            ...
            <time dateTime={post.fullDate || post.date}>{post.fullDate || post.date}</time>
          </span>
          <span className="flex items-center gap-2 mt-1">
        ...
        {/* Back to blog CTA */}
        <div className="not-prose mt-16 pt-8 border-t border-white/10 flex flex-wrap gap-4 items-center justify-between">
          <Link href="/blog">
            <button
          ...
          <a
            href="https://wa.me/917737877978?text=Hi%20Yash!%20I%20read%20your%20blog%20and%20would%20like%20to%20inquire%20about%20availability."
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-2 px-6 py-3 bg-[#D4AF37] text-black text-xs font-bold uppercase tracking-widest rounded-full hover:bg-[#c49f2e] transition-all shadow-[0_0_20px_rgba(212,175,55,0.3)]"
          >
            Book Anchor Yash
          </a>
        </div>
