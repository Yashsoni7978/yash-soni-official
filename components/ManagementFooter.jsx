...
import {
  ...
  MessageCircle, ExternalLink, Briefcase, Twitter
} from "lucide-react";
...
export default function ManagementFooter() {
  ...
  return (
    <footer className="bg-[#0a0a0a] border-t border-[#d4af37]/20 pt-24 pb-8 relative overflow-hidden font-sans">
      ...
      <div className="container mx-auto px-6 max-w-7xl relative z-10">
        <div className="grid md:grid-cols-4 gap-12 mb-16">
          ...
          <div className="md:col-span-1 space-y-6">
            <Link href="/" className="flex flex-col items-start gap-3 group">
              <div className="p-3 rounded-xl bg-[#D4AF37]/10 group-hover:bg-[#D4AF37]/20 transition-colors">
                <Briefcase className="w-6 h-6 text-[#D4AF37] group-hover:scale-110 transition-transform" />
              </div>
              <span className="text-xl md:text-2xl font-display font-black text-white tracking-widest uppercase leading-tight">
                TEAM <br className="hidden md:block" />
                <GoldTextureText>YASH SONI</GoldTextureText>
              </span>
            </Link>
            <p className="text-gray-400 text-sm leading-relaxed font-light">
              Rajasthan's premium event management and luxury design team. Curating flawless weddings, corporate summits, and milestone celebrations.
            </p>
          ...
          {/* COL 2: INSTANT ACTIONS */}
          <div>
            <h4 className="text-[#D4AF37] font-display font-bold uppercase tracking-widest text-xs mb-8">Direct Access</h4>
          ...
          {/* COL 4: NAVIGATION */}
          <div>
            <h4 className="text-[#D4AF37] font-display font-bold uppercase tracking-widest text-xs mb-8">Explore</h4>
...
const ContactButton = ({ href, icon, label, subLabel, color }) => (
  <a
    ...
    className={`flex items-center gap-4 bg-[#111] border border-white/5 p-4 rounded-2xl transition-all duration-300 group ${color} shadow-lg`}
  >
...
const SocialIcon = ({ href, icon, label }) => (
  <a
    ...
    rel="noopener noreferrer"
    aria-label={label}
    className="w-12 h-12 rounded-full border border-white/10 bg-[#111] flex items-center justify-center text-gray-400 hover:text-black hover:bg-[#D4AF37] hover:border-[#D4AF37] transition-all duration-300 shadow-lg"
  >
    {icon}
  </a>
);
